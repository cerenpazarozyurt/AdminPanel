        ...
        <div class="d-flex justify-content-center align-items-center">
            <div class="card" style="width: 75%; background-color: #778da9; border-radius: 1rem;">
                <div class="card-body bg-light" style="margin: 1rem; border-radius: 1rem;">
                    ...
                    <form action="{{ url('panel/categories/update/' . $categories->id) }}" method="POST"
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    @php
                                        $selectedPage = $categories->page;
                                    @endphp
                                    <select id="sayfaGirisi" name="page" class="form-control">
                                        <option value="" disabled selected>Seçiniz</option>
                                        <option value="blog" {{ $selectedPage == 'blog' ? 'selected' : '' }}>Blog</option>
                                        <option value="urunler" {{ $selectedPage == 'urunler' ? 'selected' : '' }}>Ürünler</option>
                                        <option value="news" {{ $selectedPage == 'news' ? 'selected' : '' }}>News</option>
                                    </select>
                                </div>
                            </div>                            
                        </div>         
                        ...
                        <button type="submit" class="btn text-right text-decoration-none text-light"
                            style="background-color: black;">Kaydet</button>
                    </form>
                </div>
            </div>
        </div>
